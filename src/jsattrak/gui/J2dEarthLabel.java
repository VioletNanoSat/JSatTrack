...
import static name.gano.astro.MathUtils.mult;
...
public class J2dEarthLabel extends JLabel  implements java.io.Serializable
{
    ...
    public J2dEarthLabel(ImageIcon image, double aspect, Color backgroundColor,
            Time currentTime, Sun sun, J2DEarthPanel earthPanel1, JSatTrak app)
    {
        //super(image);
        aspectRatio = aspect;
        this.sun = sun;
...
//        }
     ...
     * <p>The conclusions are that drawPolyline() is about 3-6 times faster than drawLine() 
     * <p>http://www.particle.kth.se/~fmi/kurs/PhysicsSimulation/Lectures/12A/timing.html
     * <p>also replace use of polygon... to drawPolygon(int[] xPoints, int[] yPoints, int nPoints) and fillpolygon(int[]...) ... so they share the same data
     * 
     */
    private void finishPainting(Graphics g)
    {
        ...
        // search through all sat nodes
        for (AbstractSatellite sat : app.satellites()) {
            if (sat.isGroundTrackShown() && sat.getGroundTrackIni()
                    && sat.isPlot2D()) {                
                ...
                if (sat.getNumGroundTrackLeadPts() > 1) {
                    ...
                    xy_old = findXYfromLL(toDegrees(LLA[0]), toDegrees(LLA[1]), w, h, imageWidth, imageHeight);
                    LLA_old[0] = LLA[0];
                    LLA_old[1] = LLA[1];
                    //LLA_old[2] = LLA[2];
                    ...
                    for (int j = 1; j < sat.getNumGroundTrackLeadPts(); j++) {
                        ...
                        xy = findXYfromLL(toDegrees(LLA[0]), toDegrees(LLA[1]), w, h, imageWidth, imageHeight);
                        
                        // check to see if Longitude flipped sign But not near origin
                        //if( (LLA[1] > 0 && LLA_old[1] > 0) || (LLA[1] < 0 && LLA_old[1] < 0) || Math.abs(LLA[1]) < 1.0  )
                        if ( !Double.isNaN(LLA_old[0]) || Double.isNaN(LLA[0])) {// make sure they are not NAN (not in time)
        ...
        // draw current positions TODO Why can't this be done in the previous loop?
        for(AbstractSatellite sat : app.satellites() ) { // search through all sat nodes
            if( sat.isPlot2D() ) {// if option to plot is on
                ...
                if(sat.isPlot2DFootPrint()){ // draw footprint, if desired
                    drawFootPrint(g2, sat, 0.2f);
                }
        ...
        if (showFPS) {
            g2.setPaint( dateTimeColor );
            g2.drawString( "FPS: " +  df.format(earthPanel.getApp().getFpsAnimation()),getWidth()-75,(int)((getHeight()-imageHeight)/2.0+imageHeight-yDateTimeOffset));
        }
    } // finish painting
    

    /**
     * Function to find the Linearly interpolated latitude at long = +/- 180. this is used to correct discontinuities in the plots
     * @return latitude for the longitude of +/- 180 (both)
     */
    ...
    /**
     *  includes calculations for zoom Facter, central lat and central longitude- input degrees
     ...
     * @return
     */
    public static int[] findXYfromLL(double lat, double lon, int totWidth,
     ...
     * @return
     */
    public static double[] findLLfromXY(int x, int y, int totWidth, int totHeight,
    ...
    private void drawFootPrint(Graphics2D mG2, AbstractSatellite mSat,
            float alpha) {
        drawFootPrint(mG2, mSat.getLatitude(), mSat.getLongitude(), mSat
                .getAltitude(), mSat.isFillFootPrint(), mSat.getSatColor(), mSat
                .getSatColor(), alpha, mSat.getNumPtsFootPrint());
    }
    
    /** =======================================================================
     * FUNCTION TO DRAW A FOOT PRINT ==========================================
     * <p>alpha = 0-1 amount of transparency in footprint (~0.2f)
     * <p>Assumes a spherical Earth? - maybe not {@see GeoFunctions#ecef2lla_Fast()}
     */
