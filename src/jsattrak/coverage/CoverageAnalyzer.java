...
import jsattrak.gui.J2dEarthLabel;
...
//        CoverageAnalyzer ca = new CoverageAnalyzer();
    ...
    public void updateTime(final Time currentJulianDate, 
            final HashMap<String, AbstractSatellite> satHash, 
            final HashMap<String, GroundStation> gsHash)
    {
        if(!dynamicUpdating)
        {
     ...
     * @param satHash
     */
    public void performCoverageAnalysis(final Time currentJulianDate, final HashMap<String,AbstractSatellite> satHash)
    {
...
//                }
                ...
                do
                {
                    ...
                    aer = AER.calculate(new double[]{getLatPanelMidPoints()[i],getLonPanelMidPoints()[longIndex],0}, 
                        currentSat.getTEMEPos(),
                        currentJulianDate.getMJD());
                    ...
                    if(aer[1] >= elevationLimit)
                    {
                        ...
                        do
                        {
                            ...
                            tempElevation2 = AER.calculate(new double[]{getLatPanelMidPoints()[i],getLonPanelMidPoints()[jWrappedIndex],0}, 
                                currentSat.getTEMEPos(),
                                currentJulianDate.getJulianDate())[1];
                ...
                if (i >= 0) // avoid searching down if i is already 0
                {
                    ...
                    {
                        // take care of when i >= latPanels (reflection for longitude index and make lat go down instead of up (and stay at top one iter)
                        aer = AER.calculate(new double[]
                        {
                            getLatPanelMidPoints()[i], getLonPanelMidPoints()[longIndex], 0
                        
                        },
                                currentSat.getTEMEPos(), // sea level
    ...
    // draw 2d
    public void draw2d(Graphics2D g2, J2dEarthLabel earthLabel, int totWidth, int totHeight, int imgWidth, int imgHeight, double zoomFac, double cLat, double cLong)
    {
        int[] xy = new int[2];
        ...
        if (plotCoverageGrid)
        {
            ...
            {
                xy = J2dEarthLabel.findXYfromLL(lat, longBounds[0], totWidth, totHeight, imgWidth, imgHeight, zoomFac, cLat, cLong);
                xy_old = J2dEarthLabel.findXYfromLL(lat, longBounds[1], totWidth, totHeight, imgWidth, imgHeight, zoomFac, cLat, cLong);
            ...
            {
                xy = J2dEarthLabel.findXYfromLL(latBounds[0], lon, totWidth, totHeight, imgWidth, imgHeight, zoomFac, cLat, cLong);
                xy_old = J2dEarthLabel.findXYfromLL(latBounds[1], lon, totWidth, totHeight, imgWidth, imgHeight, zoomFac, cLat, cLong);
            ...
            for (double lat : getLatPanelMidPoints())
            {
                ...
                {
                    xy = J2dEarthLabel.findXYfromLL(lat, lon, totWidth, totHeight, imgWidth, imgHeight, zoomFac, cLat, cLong);
        ...
        for(int i=0;i<getLatPanelMidPoints().length;i++)
        {
            ...
            {
                xy = J2dEarthLabel.findXYfromLL(getLatGridPoints()[i], getLonGridPoints()[j],totWidth,totHeight,imgWidth,imgHeight,zoomFac,cLat,cLong);
                xy_old = J2dEarthLabel.findXYfromLL(getLatGridPoints()[i+1], getLonGridPoints()[j+1],totWidth,totHeight,imgWidth,imgHeight,zoomFac,cLat,cLong);
    ...
    {
        return copyOf(latBounds, latBounds.length);
    }

    public void setLatBounds(double[] latBounds)
    {
        this.latBounds = copyOf(latBounds, latBounds.length);
    }

    public double[] getLongBounds()
    {
        return copyOf(longBounds, longBounds.length);
    }

    public void setLongBounds(double[] longBounds)
    {
        this.longBounds = copyOf(longBounds, longBounds.length);
    }
    ...
    {
        return Arrays2d.copyOf(coverageCumTime, coverageCumTime.length);
    }

    public double[] getLatPanelMidPoints()
    {
        return copyOf(latPanelMidPoints, latPanelMidPoints.length);
    }

    public double[] getLonPanelMidPoints()
    {
        return copyOf(lonPanelMidPoints, lonPanelMidPoints.length);
    }

    public double[] getLatGridPoints()
    {
        return copyOf(latGridPoints, latGridPoints.length);
    }

    public double[] getLonGridPoints()
    {
        return copyOf(lonGridPoints, lonGridPoints.length);
    }
