...
public class GatewayMgr {
	...
	public void tncTxTest() {
		if (tncStatus() != ServerStatus.DEAD) {
			...
			int numOfPackets = 1;		// number of packets to be sent
			int transmitPeriod = 400;   // time in milliseconds loop sleeps before sending next packet
			int packetLength = 18;//255-16;
			
			for(int i = 0; i < numOfPackets; i++){
				byte[] testPkt = new byte[packetLength];
				//String txDataStr = "Callsign KC2UGM Tes ting 1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZabdefghijklmnopqrstuvwxyz!@#$%^&*()-_=+[]";
				byte[] txData = new byte[packetLength - 15]; // -64 is C0 and -37 is DB
				txData[0] = 24;
				txData[1] = -1;
				for(int j = 2; j < packetLength - 16; j++)
					//txData[j] = (byte) Math.floor(Math.random()*256-128); // -64 is C0 and -37 is DB
					txData[j] = (byte) -1;
				txData[packetLength - 16] = (byte) ((i+1)%128);
				//txDataStr.getBytes();
	...
	public void tncRxTest() {
		if (tncStatus() != ServerStatus.DEAD) {
			...
			System.arraycopy(Callsign.GROUND.toArray(true), 0, testPkt, 0,
					Callsign.length());
			System.arraycopy(Callsign.VIOLET.toArray(false), 0, testPkt, Callsign
					.length(), Callsign.length());
			System.arraycopy(txDataStr.getBytes(), 0, testPkt,
					2*Callsign.length(), txDataStr.getBytes().length); //316-2*Callsign.length());
			tnc.receive(testPkt);
