...
//    JDialog parentExternal; // parent dialog
     ...
     * @param app 
     */
    public J3DEarthPanel(JDialog parent, double currentMJD, JSatTrak app)
    {
        this.parent = parent;
        this.app = app;
        ...
        for (Layer layer : m.getLayers())
        {
            ...
            {
                layer.setEnabled(false); // off by default
            }
        } // for layers
        ...
        orbitModel = new OrbitModelRenderable(app, wwd.getModel().getGlobe());
        ...
        ecefModel = new ECEFModelRenderable(app, wwd.getModel().getGlobe());
...
//        }
    ...
    {//GEN-HEADEREND:event_screenCaptureButtonActionPerformed
        ...
        Point pt = wwd.getLocationOnScreen();
        int width = wwd.getWidth();
        int height = wwd.getHeight();

        // not a possible size
        if (height <= 0 || width <= 0) { // no screen shot
        ...
        try {
            ...
            JOptionPane.showInternalMessageDialog(app.mainDesktopPane,
...
//    }
    ...
    private static class LayerAction extends AbstractAction
    {
		...
		private static final long serialVersionUID = -3811379638194745193L;
		private Layer layer;
...
//            }
        ...
        if (terrainProfileLayer.isEnabled())
        {
            try
            {
                AbstractSatellite sat = app.getSatellite(terrainProfileSat);
...
//        see result and text output in log console
        ...
        AbstractSatellite sat = app.getSatellite("ISS (ZARYA)             ");
