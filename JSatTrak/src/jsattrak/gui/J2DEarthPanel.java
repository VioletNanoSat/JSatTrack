...
public class J2DEarthPanel extends JPanel implements ComponentListener,
    ...
    private static final long serialVersionUID = -2416463385151802546L;
    ...
    private transient JMenuItem latlonLinesMenu, moreOptionsMenu; // ,
                                                                    // earthPeMenu,
                                                                    // earthNoaaMenu
    ...
    private transient BufferedImage bimageScaled; // stores scaled Earth image
                                                    // map - added for Earth
                                                    // Lights Option
    
    private String backgroundImagePath = "/images/Earth_PE_small.jpg"; // default
                                                                        // image
    ...
    // edit jsattrak.utilities.LandMassRegions to load either US map or World
    // Map
    LandMassRegions landMass = new LandMassRegions();
    int indexSpacing = 1; // index increment for plotting regions (1=best, >1
                            // faster)
...
//    Color landOutlineColor = Color.WHITE;
    ...
    // current mask of earth lights
    private transient BufferedImage earthLightsCurrentMask; 
    // saves path to earth lights image in JAR
    private String earthLightsMaskImagePath = "/images/earth_lights_lrg.jpg"; 
    ...
    public J2DEarthPanel(
            JToggleButton zoomInToggleButton,
            JToggleButton zoomOutToggleButton,
            JToggleButton recenterToggleButton, Time currentTime, Sun sun,
            JSatTrak app) {
        ...
        // TEMP try out the LandMassRegions -- move this to top class, so each
        // map doesn't have their own? or leave it this way so each can be
        // customized
...
//        }
    ...
    private void iniObject() {
        ...
        imageMap = new J2dEarthLabel(planetImage, aspectRatio,
                backgroundColor, currentTime, sun, this, app);
        imageMap.setHorizontalAlignment(JLabel.CENTER);
        //JScrollPane tableScrollPane = new JScrollPane(imageMap);
        
        super.addComponentListener(this); // adds the listner to the whole
                                            // internal window
        ...
        ActionListener menuListener = new ActionListener() {
            public void actionPerformed(ActionEvent event) {
                ...
                if (event.getSource() == latlonLinesMenu) {
                    ...
                    imageMap.showLatLong = !imageMap.showLatLong; // toogle lat
                                                                    // long
                    imageMap.repaint();
                } 
...
//                    backgroundImagePath = "/images/Earth_PE_small.png";
                // ImageIcon temp =
                // createImageIcon(backgroundImagePath,"Earth");
                // //ImageIcon temp =
                // createImageIcon(Toolkit.getDefaultToolkit().getImage(getClass().getResource("../images/Earth_PE_small.png")),"Earth");
...
//                }
        ...
        latlonLinesMenu = new JMenuItem("Toggle Lat/Lon"); // , new
                                                            // ImageIcon("1.gif")
        ...
        // checkMark =
        // createImageIcon("/icons/other/emblem-default.png","check");
//        
...
//        popup.addSeparator();
     ...
     * Dimension dim = getSize(); int w = dim.width; int h = dim.height;
     * 
     * //System.out.println("test"); g2.setPaint(Color.black); //g.fill
  ...
  */
    ...
    public ImageIcon createImageIcon(String path, String description) {
        ...
        if (imgURL != null) {// first check in the JAR
            return new ImageIcon(imgURL, description); 
        }
        if ((new File(path)).exists()) // see if file exists on system
        { // load from file system
            return new ImageIcon(path, description);
            }
            
            // Can't find file so revert to default - stored in JAR!
        System.out.println("Couldn't find 2d pixmap file: " + path
...
//       g2.fillRect(0, 0, SIDE1, SIDE2);
        ...
        for (int t = thickness; t >= 1; t = t - 2) {
            ...
            // newStroke = new BasicStroke(t, BasicStroke.CAP_ROUND,
            // BasicStroke.JOIN_ROUND);
            newStroke = new BasicStroke(t, BasicStroke.CAP_BUTT,
                    BasicStroke.JOIN_ROUND);
     ...
     /**
     * Takes value from blue channel and copies it to all color channels
     * including alpha channel meant to take a black and white image and using
     * the black level as the transparency level
     * 
     * @param dimg
     *            buffered image
    */
...
//        g.dispose();
        for (int i = 0; i < dimg.getHeight(); i++) {
            for (int j = 0; j < dimg.getWidth(); j++) {
                ...
                int blue = dimg.getRGB(j, i) & 0x000000FF; // current blue value
                                                            // (should be same
                                                            // as other
                                                            // channels)
...
//		int b2 = (dimg.getRGB(j, i) ) & 0xff;
   ...
   // assumes both day and night image are already scaled properly, 
    // this just computes a new image mask based on the current time and saves
    // the image to
   // icon setting.
    // THIS IS VERY SLOW (AND MEMORY CONSUMING) -- SEEMS LIKE THERE SHOULD BE A
    // WAY TO IMPROVE
    public void updateEarthLightMaskAndRecombineImage() {
       ...
       if(this.isShowEarthLightsMask()) // need to scale Earth Lights mask 
       {
           // copy earthLightsCurrentMask so mask isn't applied over and over
            BufferedImage earthLightsCurrentMaskTemp = new BufferedImage(
                    earthLightsCurrentMask.getWidth(), earthLightsCurrentMask
           ...
           gg.dispose();
            ...
            BufferedImage dayNightMask = new BufferedImage(
                    earthLightsCurrentMask.getWidth(), earthLightsCurrentMask
                            .getHeight(), BufferedImage.TYPE_INT_ARGB);
            Graphics2D maskG2D = dayNightMask.createGraphics();
            maskG2D.setColor(new Color(255, 255, 255)); // white
            // fill background with white
            maskG2D.fillRect(0, 0, earthLightsCurrentMask.getWidth(),
                    earthLightsCurrentMask.getHeight());
            
            // get the shape of the dark region
            double[] lla = sun.getCurrentDarkLLA();
            Polygon[] pgons = imageMap.getFootPrintPolygons(lla[0], lla[1],
                    lla[2], imageMap.getNumPtsSunFootPrint());
            
            // draw lines around edges
            int shadowThickness = (int) (imageMap.getZoomFactor()
                    * earthLightsCurrentMask.getWidth() / 20.0);// MAKE THIS
                                                                // SETABLE
            for (Polygon p : pgons) {
            ...
            // last step, now take black and white image and make alpha channel
            // that matches black intensity
            // had to do this because otherwise when drawing fuzzy line (over
            // and over) tansparency adds up
            ...
            topG2D.drawImage(dayNightMask, 0, 0, earthLightsCurrentMask
                    .getWidth(), earthLightsCurrentMask.getHeight(), null);
            topG2D.dispose();      
            ...
            // g.drawImage(earthLightsCurrentMask,0,0,null);
            // //earthLightsCurrentMask ,earthLightsFullRes
            g.dispose();
    ...
    public void rescaleAndSetBackgroundImage() {
        ...
        // rescales background image and if neeeded earth lights and recreates
        // background image
        ...
        // ALSO NEED A FUNCTION that doesn't scale... just updates image because
        // Earth Light effect is enambled and new background image is required
        
    }
    
    // overloaded - default to RGB image
    public BufferedImage scaleImageMap(BufferedImage fullResImage) {
        return scaleImageMap(fullResImage,
                java.awt.image.BufferedImage.TYPE_INT_RGB);
    }
    
    /**
     * Scales the image map based on current window size - maintains aspect
     * ratio of map, returns correctly scaled image. Also auto sets the imagemap
     * lable the current scale size. Also scales for appropriate zoom level and
     * center.
     * 
     * @param fullResImage
     *            Input image that is full res.
     * @param imageType
     *            image type
    ...
    public BufferedImage scaleImageMap(BufferedImage fullResImage, int imageType) {
        ...
        if (imageMap.getHeight() != 0) {
            ...
            if (imageMap.aspectRatio() > aspectRatio) {
                newHeight = imageMap.getHeight();
                newWidth = (int) (imageMap.getHeight()*aspectRatio);
            } else { // label has lower aspect ratio
                newWidth = imageMap.getWidth();
                newHeight = (int) (imageMap.getWidth()*1.0/aspectRatio);
            }
            
            // take into account zoom: (might not be the best way as it can
            // create a huge image in memory... but okay for now)
            // in future may want to calculate the part of the image that will
            // be displayed an clip the rest somehow
            ...
            // BufferedImage midImage = bimage.getSubimage(int x, int y, int w,
            // int h);
            // gunna need orginal image size, new image size, total image
            // size... etc.
            // center lat,long... zoomfactor.
            
            int midHeight = (int) Math.round(fullResImage.getHeight()
                    / imageMap.getZoomFactor());
            int midWidth = (int) Math.round(fullResImage.getWidth()
                    / imageMap.getZoomFactor());
            // uper left corner of image
            int midXupLeft = (int) Math
                    .round((fullResImage.getWidth() - midWidth) / 2.0
                            + fullResImage.getWidth()
                            * imageMap.getCenterLong() / 360.0);
            int midYupLeft = (int) Math
                    .round((fullResImage.getHeight() - midHeight) / 2.0
                            - fullResImage.getHeight()
                            * imageMap.getCenterLat() / 180.0);
            ...
            if(midXupLeft < 0) // too close to left side
            {
                ...
                // new center long value: (solve midXupLeft == 0, for
                // centerLong)
                double newCenterLong = (0.0 - (fullResImage.getWidth() - midWidth) / 2.0)
                        * 360.0 / fullResImage.getWidth();
                ...
                // solve midXupLeft + midWidth == fullResImage.getWidth(), for
                // centerLong
                // too close to right side
                double newCenterLong = ((fullResImage.getWidth() - midWidth) - (fullResImage
                        .getWidth() - midWidth) / 2.0)
                        * 360.0 / fullResImage.getWidth();
                imageMap.setCenterLong( newCenterLong );
            ...
            {
                // midYupLeft = (fullResImage.getHeight()-midHeight)/2.0 -
                // fullResImage.getHeight()*imageMap.getCenterLat()/180.0;
                double newCenterLat = -((0.0) - (fullResImage.getHeight() - midHeight) / 2.0)
                        * 180.0 / fullResImage.getHeight();
                ...
                // too close to bottom
                double newCenterLat = -((fullResImage.getHeight() - midHeight) - (fullResImage
                        .getHeight() - midHeight) / 2.0)
                        * 180.0 / fullResImage.getHeight();
                imageMap.setCenterLat( newCenterLat );
            ...
            BufferedImage midImage = fullResImage.getSubimage(midXupLeft,
                    midYupLeft, midWidth, midHeight);
...
//            }
            ...
            if (landMass.isShowLandMassOutlines()) {
                ...
                Image scaledImage = midImage.getScaledInstance(newWidth,
                        newHeight, imageScalingOption);
    ...
    public BufferedImage getBufferedImage(ImageIcon img) {
        ...
        if(!GraphicsEnvironment.isHeadless()) {
            ...
            bimageIN = gc.createCompatibleImage(image.getWidth(null), image
        ...
        if (bimageIN == null) {
            ...
            bimageIN = new BufferedImage(image.getWidth(null), image
                    .getHeight(null), type);
        }
...
//  An inner class to check whether mouse events are the popup trigger
    class MousePopupListener extends MouseAdapter {
        ...
        @Override public void mousePressed(MouseEvent e) {
            ...
            // do something here to see if zoom was enabled.. and not right
            // click
            ...
            if(e.getButton() == 2) // middle button prints out lat/long
            {
                ...
                double[] ll = imageMap.findLLfromXY(e.getX(), e.getY(),
                        imageMap.getLastTotalWidth(), imageMap
                                .getLastTotalHeight(),
                        imageMap.getImageWidth(), imageMap.getImageHeight());
            ...
            if (e.getButton() == 1 && zoomInToggleButton.isSelected()) {
                ...
                // new zoom factor
                double newZoomFactor = imageMap.getZoomFactor()
                        * imageMap.getZoomIncrementMultiplier();
            ...
            if (e.getButton() == 1 && zoomOutToggleButton.isSelected()) {
                ...
                if (imageMap.getZoomFactor() <= 1.00001) // don't let it be less
                                                            // than 1
                {
        ...
        @Override public void mouseDragged(MouseEvent e) {
            ...
            double[] ll = imageMap.findLLfromXY(e.getX(), e.getY(), imageMap
                    .getLastTotalWidth(), imageMap.getLastTotalHeight(),
                    imageMap.getImageWidth(), imageMap.getImageHeight());
    ...
    static class PopupPrintListener implements PopupMenuListener {
...
//            case 0:
        // ImageIcon temp =
        // createImageIcon("/images/Earth_PE_small.png","Earth");
//                bimage = getBufferedImage(temp);
//                ScaleImageMap(); // repaint the image
//                earthNoaaMenu.setIcon(null);
//                earthPeMenu.setIcon(checkMark);
...
//        {
    // ImageIcon temp =
    // createImageIcon("/images/Earth_NOAA_NGDC_small.png","Earth");
//            bimage = getBufferedImage(temp);
//            ScaleImageMap(); // repaint the image
//            earthNoaaMenu.setIcon(checkMark);
//            earthPeMenu.setIcon(null);
...
//    } // setImageMapNum
    ...
    private void drawLandMasses(Graphics2D g2, int width, int height) {
        ...
        if (landMass.isShowLandMassOutlines()) {
            ...
            // drawing disconnects across international date line not accounted
            // for
            for (String key : landMass.getLandMassHash().keySet()) {
                ...
                if (vec.size() > 0) {
                    ...
                    // int[] lastXY = imageMap.findXYfromLL(lastLL[0],
                    // lastLL[1], w, h, imageWidth, imageHeight);
                    int[] lastXY = imageMap.findXYfromLL(lastLL[0], lastLL[1],
                    ...
                    for (int i = 1; i < vec.size(); i += indexSpacing) {
                        ...
                        // currXY = imageMap.findXYfromLL(currLL[0], currLL[1],
                    ...
                    currXY = imageMap.findXYfromLL(currLL[0], currLL[1], width,
                            height, width, height);
                    
                    // only draw end point connection if not too far away
                    if (Math.sqrt(Math.pow(currXY[0] - lastXY[0], 2)
                            + Math.pow(currXY[1] - lastXY[1], 2)) > 15) {
...
//    }
    ...
    public void setRegionDrawingOptions(boolean bool, String fileName,
            Color color) {
        // save old data
        ...
        // see if any changes
        if (!oldName.equalsIgnoreCase(fileName) || preBool != bool
                || prevColor != color) {
            rescaleAndSetBackgroundImage(); // repaint the image
        }
    ...
    public void setShowEarthLightsMask(boolean showEarthLights) {
        ...
        if (showEarthLights != isShowEarthLightsMask()) {
            ...
            {
                // do we need to load the image? (could have already been
                // loaded)
                if (earthLightsFullRes == null) {
                    ImageIcon lightsImage = createImageIcon(
                            earthLightsMaskImagePath, "Earth Lights");
            ...
            {
                // remove data
                // earthLightsFullRes = null; // delete // save in case we need
                // it later
    ...
    public J2dEarthLabel getImageMap() {
        return imageMap;
    }
    
    /**
     * Performs an offscreen rendering of panel, works best when used with
     * "getNonDisplayed2DEarthPanel()" function in JSatTrak class
     * 
     * @return
     */
    public BufferedImage offScreenRender() {
        BufferedImage buff = new BufferedImage(this.getWidth(), this
                .getHeight(), BufferedImage.TYPE_INT_RGB);
