...
import static name.gano.file.FileUtilities.deleteDirectory;
...
public class JCreateMovieDialog extends javax.swing.JDialog
{
    ...
    private void iniGUI(String windowTitle)
    {
        ...
        startTime.setDateFormat( (SimpleDateFormat)app.getCurrentJulianDate().getDateFormat());//threeDpanel.getApp().getCurrentJulianDay().getDateFormat() ); 
        startTime.set( app.getCurrentJulianDate().getCurrentGregorianCalendar().getTimeInMillis()  );
        // End time
        endTime = new Time();
        endTime.setDateFormat( (SimpleDateFormat)app.getCurrentJulianDate().getDateFormat() ); 
        endTime.set( app.getCurrentJulianDate().getCurrentGregorianCalendar().getTimeInMillis()  );
    ...
    {//GEN-HEADEREND:event_genMovieButtonActionPerformed
        ...
        final FileFilter movFilter = new FileNameExtensionFilter("mov", "*.mov");
        ...
        if (fc.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
            ...
            if (extension == null) { // We add an extension based on type
                file = new File(file.getAbsolutePath() + ".mov");
            //System.out.println("path="+file.getAbsolutePath());
            }
            
            outputMoviePath = "file:" + file.getAbsolutePath();

        } else return; // don't do anything more - user canceled
        
        // === CREATE temporary files ========================
        
        // create temp directory
        if(!(new File(tempDirStr)).mkdir()){
            System.err.println("Unable to create directory: " + tempDirStr);
            return;
        }
        ...
        new SwingWorker<Object, Integer>() {
            ...
            public Vector<String> doInBackground() {
                ...
                for (int i = 0; i < numFrames; i++) {
                    app.setTime(startTime.getCurrentGregorianCalendar()
                            .getTimeInMillis());

                    // force component to update
                    if (movieMode == 0) {
                        threeDpanel.getWwd().redrawNow(); // // force wwd to
                                                          // update
                    } else if (movieMode == 1) {
                        // is this needed, seems no; well seems to help remove
                        // some black flickering artifacts
                        // twoDpanel.repaint();
                    } else {
                    ...
                    // take screen shot
                    createScreenCapture(tempDirStr + "/" + rootNameTempImages
                            + i + ".jpg");
                    // save file name
                    inputFiles.addElement(tempDirStr + "/" + rootNameTempImages
                            + i + ".jpg");

                    publish((int) (100.0 * (i + 1.0) / numFrames));
                    // movieStatusBar.setValue((int) (100.0 * (i + 1.0) /
                    // numFrames));
                ...
                if ((oml = createMediaLocator(outputMoviePathFinal)) == null) {
                    JOptionPane.showMessageDialog(null,
                            "ERROR Creating Output File (check permissions)",
                            "ERROR", JOptionPane.ERROR_MESSAGE);
                    System.err.println("Cannot build media locator from: "
                            + outputMoviePathFinal);
            ...
            // runs every once in a while to update GUI, use publish( int ) and
            // the int will be added to the List
            @Override
...
//                                  currentTimeDate.get(Calendar.SECOND));
    ...
    public void createScreenCapture(String outputFileName)
    {
        try
        {
            ...
            try{
            	SaveImageFile.saveImage(fileExtension, file, screencapture, 0.9f); // the last one is the compression quality (1=best)
            } catch(Exception e){
