...
public class JSatBrowserInternalFrame extends javax.swing.JInternalFrame {
    ...
    /**
     * data loader For now custom TLE data loaded just loads for the current
     * broswer, after it is closed, a new browser won't have the data
     */
     ...
     * @param app
     */
    public JSatBrowserInternalFrame(JFrame parent, JSatTrak app) {
        ...
        tleHash = new HashMap<String, TLE>();

        // create tree model using root node
        ...
        if ((new File(tleDownloader.getLocalPath()).exists())
            ...
            while (!sbtdl.isDone()) {
             // sleep for a little bit to wait for the thread
                try {Thread.sleep(50); 
    ...
    {// GEN-HEADEREND:event_satTreeValueChanged
        ...
        if (satTree.getSelectionCount() > 0) {
            if (tleHash.containsKey(satTree.getLastSelectedPathComponent()
                    .toString())) {
                TLE selectedTLE = tleHash.get(satTree
    ...
    {// GEN-HEADEREND:event_loadCustomTLEMenuItemActionPerformed
        ...
        final JFileChooser fc = new JFileChooser(new TLEDownloader()
                .getTleFilePath(0));
        fc.addChoosableFileFilter(new FileNameExtensionFilter("tle", "*.tle"));
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            if(sbtdl.loadTLEDataFile(file, "Custom", null)){
     ...
     * @return
     */
    public HashMap<String, TLE> getTleHash() {
        return tleHash;
    }
